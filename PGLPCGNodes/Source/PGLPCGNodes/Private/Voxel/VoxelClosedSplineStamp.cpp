...
TSharedPtr<FVoxelGraphEnvironment> FVoxelClosedSplineStamp::CreateEnvironment(
...
TSharedPtr<IVoxelParameterOverridesOwner> FVoxelClosedSplineStamp::GetShared() const
{
	return SharedThis(ConstCast(this));
}

bool FVoxelClosedSplineStamp::ShouldForceEnableOverride(const FGuid& Guid) const
{
	return true;
}

UVoxelGraph* FVoxelClosedSplineStamp::GetGraph() const
{
	return Graph;
}

FVoxelParameterOverrides& FVoxelClosedSplineStamp::GetParameterOverrides()
{
	return ParameterOverrides;
}

FProperty* FVoxelClosedSplineStamp::GetParameterOverridesProperty() const
{
	return &FindFPropertyChecked(FVoxelClosedSplineStamp, ParameterOverrides);
}
...
UObject* FVoxelClosedSplineStamp::GetAsset() const
{
	return Graph;
}

void FVoxelClosedSplineStamp::FixupProperties()
{
...
void FVoxelClosedSplineStamp::FixupComponents(const IVoxelStampComponentInterface& Interface)
{
	VOXEL_FUNCTION_COUNTER();
...
TVoxelArray<TSubclassOf<USceneComponent>> FVoxelClosedSplineStamp::GetRequiredComponents() const
{
...
bool FVoxelClosedSplineStampRuntime::Initialize(FVoxelDependencyCollector& DependencyCollector)
{
	VOXEL_FUNCTION_COUNTER();
	...
	Evaluator = FVoxelNodeEvaluator::Create<FVoxelOutputNode_OutputClosedSpline>(Environment.ToSharedRef());
	...
	{
		VOXEL_MESSAGE(Error, "ClosedSpline stamp requires a closed-loop spline");
		return false;
	}
...
bool FVoxelClosedSplineStampRuntime::Initialize_Parallel(FVoxelDependencyCollector& DependencyCollector)
{
	VOXEL_FUNCTION_COUNTER();
	...
	FVoxelClosedSplineStamp& MutableStamp = GetMutableStamp();
	...
	// Compute a single bounding box from all segment bounds + padding
	if (Segments.Num() > 0)
	{
		...
		SplineBounds = SplineBounds.Extend(Padding);
	}

	return true;
}

FVoxelBox FVoxelClosedSplineStampRuntime::GetLocalBounds() const
{
	return SplineBounds;
}

bool FVoxelClosedSplineStampRuntime::ShouldUseQueryPrevious() const
{
	return GetBlendMode() == EVoxelVolumeBlendMode::Override;
}

TVoxelInlineArray<FVoxelBox, 1> FVoxelClosedSplineStampRuntime::GetChildren() const
{
...
void FVoxelClosedSplineStampRuntime::Apply(
